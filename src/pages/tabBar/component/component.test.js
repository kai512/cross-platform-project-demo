describe('pages/tabBar/component/component.nvue', () => {
    ...
    it('视图容器', async () => {
				let panelText = await page.$('.lw-panel-text')
        // 检测首个 panel 是视图容器
        expect(await panelText.text()).toBe(
            '视图容器')
						
        // 检测首个 panel 切换展开        
        const panelH = await page.$('.lw-panel-h');
        // 不能做完全匹配，百度小程序会生成额外的class
        expect(await panelH.attribute('class')).toContain('lw-panel-h')
        await panelH.tap()
        await page.waitFor(500)
        // 已展开
        expect(await panelH.attribute('class')).toContain('lw-panel-h-on')
    })

    // it('.lw-panel', async () => {
    //   const lists = await page.$$('.lw-panel')
    //   expect(lists.length).toBe(9)
    // })

    it('.lw-panel action', async () => {
      const listHead = await page.$('.lw-panel-h')
      expect(await listHead.attribute('class')).toContain('lw-panel-h-on')
      await listHead.tap() 
      ...
      await listHead.tap()
      await page.waitFor(200)
      const item = await page.$('.lw-navigate-item')
      await item.tap()
      await page.waitFor(500)
