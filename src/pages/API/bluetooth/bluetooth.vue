<template>
	<view>
		...
		<view class="lw-padding-wrap lw-common-mt">
			...
			<view class="lw-btn-v">
				...
				<view v-if="characteristicsData.length > 0">
					<view class="lw-list_name">uuid:{{ characteristicsData[0].uuid }}</view>
					<view class="lw-list_item">
		...
		<!-- 遮罩 -->
		<view v-if="maskShow" class="lw-mask" @touchmove.stop.prevent="moveHandle" @click="maskclose">
			<scroll-view class="lw-scroll_box" scroll-y @touchmove.stop.prevent="moveHandle" @click.stop="moveHandle">
				<view class="lw-title">
				...
				>
					<view v-if="showMaskType === 'device'">
						<view class="lw-list_name">{{ item.name || item.localName }}</view>
						<view class="lw-list_item">信号强度:{{ item.RSSI }}dBm</view>
						<view class="lw-list_item">UUID:{{ item.deviceId }}</view>
						<!-- <view class="list-item" v-if="showMaskType === 'device'">
							Service数量:{{ item.advertisServiceUUIDs.length }}
						</view> -->
					</view>
					<view v-if="showMaskType === 'service'">
						<view class="lw-list_item" style="line-height:2.2;">
							UUID: {{ item.uuid }}
					...
					<view v-if="showMaskType === 'characteristics'">
						...
						<view class="lw-list_item">是否支持 read 操作:{{ item.properties.read }}</view>
						<view class="lw-list_item">
...
export default {
	...
	methods: {
		...
		startBluetoothDevicesDiscovery() {
			this.$lw.startBluetoothDevicesDiscovery({
				success: e => {
		...
		stopBluetoothDevicesDiscovery(types) {
			this.$lw.stopBluetoothDevicesDiscovery({
				success: e => {
		...
		onBluetoothDeviceFound() {
			this.$lw.onBluetoothDeviceFound(devices => {
		...
		getBluetoothAdapterState() {
			console.log('--->');
			this.$lw.getBluetoothAdapterState({
				success: res => {
					console.log(JSON.stringify(res));
		...
		getBLEDeviceCharacteristics() {
			...
			this.$lw.getBLEDeviceCharacteristics({
				// 这里的 deviceId 需要已经通过 createBLEConnection 与对应设备建立链接
				deviceId,
		...
		onBLEConnectionStateChange() {
			this.$lw.onBLEConnectionStateChange(res => {
		...
		readBLECharacteristicValue() {
			...
			this.$lw.readBLECharacteristicValue({
				// 这里的 deviceId 需要已经通过 createBLEConnection 与对应设备建立链接
				deviceId,
				// 这里的 serviceId 需要在 getBLEDeviceServices 接口中获取
				serviceId,
		...
		onBLECharacteristicValueChange() {
			// 必须在这里的回调才能获取
			this.$lw.onBLECharacteristicValueChange(characteristic => {
		...
		/**
		 * 订阅操作成功后需要设备主动更新特征值的 value，才会触发 this.$lw.onBLECharacteristicValueChange 回调。
		 */
		notifyBLECharacteristicValueChange() {
			...
			console.log(characteristicId);
			console.log(notify);
			this.$lw.notifyBLECharacteristicValueChange({
