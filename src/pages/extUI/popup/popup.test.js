describe('pages/extUI/popup/popup.nvue', () => {
	...
	let page
	beforeAll(async () => {
		// 重新reLaunch至首页，并获取首页page对象（其中 program 是lw-automator自动注入的全局对象）
	...
	it('页面渲染', async () => {
		...
		let popupItem = await popup.$('.lw-popup-dialog')
		if (process.env.UNI_PLATFORM === "mp-weixin") {
			popupItem = await popup.$('lw-popup-dialog')
		}

		const popupItemText = await popupItem.$('.lw-dialog-content-text')

		expect(await popupItemText.text()).toBe('欢迎使用 lw-popup!')
	})
	...
	it('关闭默认弹层', async () => {
		let popup = await page.$('#popupDialog')
		const tapEventMask = await popup.$('.lw-mask--hook')
		...
		const tapEventButton = await popupItem.$$('.lw-dialog-button')

		// 按钮应该为确定
		expect(await (await tapEventButton[1].$('.lw-dialog-button-text')).text()).toBe('确定')
		...
		popupItem = popupMsg
		if (process.env.UNI_PLATFORM === "mp-weixin") {
			popupItem = await popupMsg.$('lw-popup-message')
		}
		// 如果提示组件不存在，则通过
		expect(popupItem).toBeNull()
	...
	it('基本示例', async () => {
		...
		// 微信小程序不能跨组件获取元素
		let content = await popup.$('.lw-popup__wrapper-box')
		...
		let tapEventMask = await popup.$('.lw-mask--hook')
		// h5 和 小程序 差一层标签，所以要选中组件内的元素触发事件
		if (process.env.UNI_PLATFORM === "mp-weixin") {
			tapEventMask = await tapEventMask.$('.lw-transition')
		}
		...
		if (process.env.UNI_PLATFORM === "mp-weixin") {
			tapEventMask = await popup.$('.lw-mask--hook')
		}
		// 如果组件不存在，则通过
	...
	it('自定义弹出层 dialog + message', async () => {
		...
		let popupItem = await popupDialog.$('.lw-popup-dialog')

		if (process.env.UNI_PLATFORM === "mp-weixin") {
			popupItem = await popupDialog.$('lw-popup-dialog')
		}

		// 弹出层内容是否正确
		expect(await (await popupItem.$('.lw-dialog-content-text')).text()).toBe('欢迎使用 lw-popup!')
		// 标题是否正确
		expect(await (await popupItem.$('.lw-dialog-title-text')).text()).toBe('通知')
		// 检测标题颜色是否正确
		expect(await (await popupItem.$('.lw-dialog-title-text')).attribute('class')).toContain(
			'lw-popup__warn')

		const tapEventDialogButton = await popupItem.$$('.lw-dialog-button')
		// 按钮应该为取消
		expect(await (await tapEventDialogButton[0].$('.lw-dialog-button-text')).text()).toBe('取消')
		...
		popupItem = popupMsg
		if (process.env.UNI_PLATFORM === "mp-weixin") {
			popupItem = await popupDialog.$('lw-popup-message')
		}
		// 如果提示组件不存在，则通过
		expect(popupItem).toBeNull()
	...
	it('提交信息 input + 延迟关闭', async () => {
		...
		// 输入弹层标题
		expect(await (await popupItem.$('.lw-dialog-title-text')).text()).toBe('输入内容')
		const tapEventDialogButton = await popupItem.$$('.lw-dialog-button')
		// 按钮应该为取消
		expect(await (await tapEventDialogButton[1].$('.lw-dialog-button-text')).text()).toBe('确定')
	...
	it('分享示例', async () => {
		...
		let popupItem = await popupShare.$('.lw-popup-share')
		if (process.env.UNI_PLATFORM === "mp-weixin") {
			popupItem = await popupShare.$('lw-popup-share')
		}
		// 分享标题是否存在
		expect(await (await popupItem.$('.lw-share-title-text')).text()).toBe('分享到')
		let shareItem = await popupItem.$$('.lw-share-content-item')
		// 分享项内容长度
		expect(shareItem.length).toBe(6)

		let shareButton = await popupItem.$('.lw-share-button')
